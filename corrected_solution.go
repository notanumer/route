...
func main() {
	...
	// In a hexagonal grid, hexagons share sides with neighbors
	// The pattern should be like:
	// _ _ _ _ _
	// / \_/ \_/ \_/ \_/ \
	// \_/ \_/ \_/ \_/ \_/
	
	colStep := width + height  // Horizontal distance between hexagon centers
	rowStep := height + 1      // Vertical distance between row centers
	...
	for row := 0; placed < k; row++ {
		...
		// Offset for odd rows in hexagonal tiling
		offsetX := 0
		if row%2 == 1 {
			offsetX = colStep / 2
		}
		...
		for col := 0; placed < k; col++ {
			...
			// Check if hexagon fits
			if x + width + 2*height - 1 > m+1 {
				break
			}
			
			// Draw hexagon
			drawHexTile(screen, x, y, width, height)
			placed++
		}
	}
...
func drawHexTile(screen [][]rune, startX, startY, width, height int) {
	maxY := len(screen) - 1
	maxX := len(screen[0]) - 1
	...
	for i := 0; i < width; i++ {
		x := startX + height + i
		if x >= 1 && x <= maxX && startY >= 1 && startY <= maxY {
			screen[startY][x] = '_'
		}
	}
